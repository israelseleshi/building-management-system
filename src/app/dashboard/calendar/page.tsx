...
function CalendarContent() {
  ...
  const [emailTargetEvent, setEmailTargetEvent] = useState<CalendarEvent | null>(null)
  ...
  const draftCount = emailMessages.filter((m) => m.status === "draft").length
  const fallbackComposeEvent: CalendarEvent = {
    id: `compose-${dateKey(selectedDate)}`,
    title: `Building Notice - ${selectedDate.toLocaleDateString()}`,
    type: "notice",
  ...
  const composeTargetEvent = detailEvent || selectedEvents[0] || fallbackComposeEvent

  const getRecipientsForEvent = (event: CalendarEvent | null): string[] => {
    if (!event) return []
    if (event.isBuildingWide) return tenants.map((t) => t.email)
    if (event.tenantId) {
      const tenant = tenants.find((t) => t.id === event.tenantId)
      return tenant ? [tenant.email] : []
    }
    if (event.unitId) {
      return tenants.filter((t) => t.unitId === event.unitId).map((t) => t.email)
    }
    return []
  }

  const pushEmailMessage = (payload: EmailComposePayload, statusValue: EmailMessage["status"]) => {
    if (!emailTargetEvent) return
    setEmailMessages((prev) => [
      {
        id: `mail-${crypto.randomUUID()}`,
        eventId: emailTargetEvent.id,
    ...
    setToastMessage(statusValue === "sent" ? "Email sent" : statusValue === "scheduled" ? "Email scheduled" : "Draft saved")
  }
  ...
  return (
    <div className="min-h-screen flex bg-[#E9EDF3]">
                        ...
                        <div key={dateKey(d)} onClick={() => setSelectedDate(d)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => {
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <button onClick={(e) => {
                                e.stopPropagation()
                                const firstEvent = (eventsByDay.get(dateKey(d)) || [])[0] || null
                                const nextTarget: CalendarEvent = firstEvent || {
                                  id: `compose-${dateKey(d)}`,
                                  title: `Building Notice - ${d.toLocaleDateString()}`,
                                  type: "notice",
                                ...
                                setSelectedDate(d)
                                setEmailTargetEvent(nextTarget)
                                setEmailModalOpen(true)
                              }} className="h-5 w-5 rounded bg-slate-100 text-slate-600 inline-flex items-center justify-center"><Mail className="h-3.5 w-3.5" /></button>
                            </div>
                          </div>
                            ...
                            {list.slice(0, 3).map((e) => (
                              <button key={e.id} draggable={!e.autoGenerated} onDragStart={(de) => de.dataTransfer.setData("event-id", e.id)} title={`${e.title}${e.description ? ` - ${e.description}` : ""}`} onClick={(ev) => { ev.stopPropagation(); setDetailId(e.id) }} className="w-full truncate rounded px-2 py-1 text-left text-[10px] font-medium text-white" style={{ backgroundColor: eventTypeColor[e.type] }}>
                                <span className="inline-flex items-center gap-1">
                                  {e.title}
                                  {emailMessages.some((m) => m.eventId === e.id && m.status !== "sent") && (
                                    <span className="inline-flex h-3.5 min-w-[14px] items-center justify-center rounded-full bg-white/90 px-1 text-[9px] font-bold text-slate-800">!</span>
                                  )}
                                </span>
                              </button>
                            ))}
      ...
      {detailEvent && (
        <div className="fixed right-5 bottom-20 z-50 w-[360px] rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl">
          <div className="flex items-center justify-between">
          ...
          <div className="mt-3 flex gap-2">
            <Button variant="outline" className="h-8 px-3 text-xs" onClick={() => { setEmailTargetEvent(detailEvent); setEmailModalOpen(true) }}>
              Notify Tenant(s)
            </Button>
            <Button variant="outline" className="h-8 px-3 text-xs" onClick={() => { setEmailTargetEvent(detailEvent); setEmailModalOpen(true) }}>
              Compose Email
          ...
          <div className="mt-4 border-t border-slate-200 pt-3">
            <div className="text-[11px] font-bold uppercase tracking-[0.08em] text-slate-600">Communication History</div>
            <div className="mt-2 max-h-[130px] space-y-2 overflow-y-auto">
              {emailMessages.filter((m) => m.eventId === detailEvent.id).map((m) => (
                <div key={m.id} className="rounded-lg border border-slate-200 p-2">
                  <div className="text-[11px] font-semibold text-slate-700">{m.subject}</div>
                  <div className="mt-1 text-[10px] text-slate-500">{m.status.toUpperCase()} • {m.createdAt.toLocaleString()}</div>
                </div>
              ))}
              {emailMessages.filter((m) => m.eventId === detailEvent.id).length === 0 && (
      ...
      <EmailModal
        open={emailModalOpen}
        onClose={() => { setEmailModalOpen(false); setEmailTargetEvent(null) }}
        event={emailTargetEvent ? { id: emailTargetEvent.id, title: emailTargetEvent.title, type: emailTargetEvent.type, startDate: emailTargetEvent.startDate } : null}
        defaultRecipients={getRecipientsForEvent(emailTargetEvent)}
        tenantOptions={tenants.map((t) => ({ id: t.id, name: t.name, email: t.email, unitName: units.find((u) => u.id === t.unitId)?.name }))}
        onSaveDraft={(payload) => pushEmailMessage(payload, "draft")}
        onSchedule={(payload) => pushEmailMessage(payload, "scheduled")}
        onSendNow={(payload) => pushEmailMessage(payload, "sent")}
      />

      {toastMessage && (
        <div className="fixed top-5 right-5 z-[80] rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white shadow-lg">
          {toastMessage}
        </div>
      )}
    </div>
...
function Widget({ title, value, tone }: { title: string; value: number; tone: "blue" | "emerald" | "rose" }) {
  const tones: Record<"blue" | "emerald" | "rose", string> = {
    blue: "bg-gradient-to-br from-[#EAF4FF] to-[#DCEBFF] text-[#0F4C81]",
    emerald: "bg-gradient-to-br from-[#E9FFF4] to-[#D7F8E8] text-[#0B6B44]",
    rose: "bg-gradient-to-br from-[#FFEFF1] to-[#FFE1E6] text-[#8E1C3A]",
  }

  return (
    <div className={`rounded-2xl p-4 text-center shadow-[0_8px_24px_rgba(31,53,73,0.08)] ${tones[tone]}`}>
      <div className="text-xs font-bold uppercase tracking-[0.08em]">{title}</div>
